...
public class DbHandler extends SQLiteOpenHelper {
    ...
    private static final String DB_NAME = "fit.db";
    ...
    // Adding new User Details
    public void updateData(int type, int flags, long timestamp, int value, int value2){
        //Get the Data Repository in write mode
        SQLiteDatabase db = this.getWritableDatabase();
        //Create a new map of values, where column names are the keys
        ContentValues cValues = new ContentValues();
        ...
        cValues.put(KEY_ACTIVITY_VALUE2, value2);
        // Insert the new row, returning the primary key value of the new row
        long newRowId = db.update(TABLE_ACTIVITY,cValues, "timestamp=" + timestamp, null);
        db.close();
    }
